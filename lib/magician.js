...
Magician = (function() {
  ...
  Magician.prototype.resizeTo = function(width, height, callback) {
    ...
    return exec("convert -resize " + width + "x" + height + " " + this.sourcePath + " " + this.destPath, function() {
      if (callback) {
  ...
  Magician.prototype.cropFrom = function(x, y, width, height, callback) {
    ...
    return exec("convert " + this.sourcePath + " -crop " + width + "x" + height + "+" + x + "+" + y + " " + this.destPath, function() {
      if (callback) {
        return callback(false);
      }
    });
  ...
  Magician.prototype.convert = function(callback) {
    return exec("convert " + this.sourcePath + " " + this.destPath, function() {
      if (callback) {
        return callback(false);
      }
    });
